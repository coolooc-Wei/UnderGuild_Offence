...
### 遊戲畫面
歡迎頁
![圖片](https://hackmd.io/_uploads/r1I1skmMGg.png)
主畫面
![螢幕擷取畫面 2026-06-19 194043](https://hackmd.io/_uploads/HJlmskmzfe.jpg)
菜單頁
![螢幕擷取畫面 2026-06-19 194355](https://hackmd.io/_uploads/r1RSsk7GGg.jpg)
抽卡畫面
![圖片](https://hackmd.io/_uploads/SJF8o17MGe.png)
合成表
![圖片](https://hackmd.io/_uploads/HyLDiy7fMe.png)
戰鬥畫面
![圖片](https://hackmd.io/_uploads/B1fOsyQGGg.png)
![圖片](https://hackmd.io/_uploads/BJWtjJQMGg.png)
羈絆效果
![圖片](https://hackmd.io/_uploads/S1RYokXzfx.png)
...
- **說明**：遊戲主迴圈透過明確的 `GameState`（如 WELCOME, MENU, GAMING, PAUSE, SETTLING, END）來嚴格控制每個階段該做什麼事。例如在 `SETTLING` 狀態下，會暫停玩家輸入與 AI 運算，專心處理掉落物的吸收動畫。這不僅提高了審查石的閱讀性，也在後期插入、變更段落邏輯石的開發更有效率。
...
- **說明**：不直接在每個 `Update` 內抓取系統 `DeltaTime` 的粗暴作法，專案實作了全局的 `Tick` 管理。當遊戲進入 `PAUSE` 狀態時，`App` 只需要停止呼叫 `AdvanceTick()`，畫面上所有依賴此時間軸的動畫、技能冷卻與 AI 就會自然凍結，無需在每一個物件內都寫入 `if (!paused)` 的冗餘判斷。
...
3. **分支管理與追蹤**：
   ...
   - **解法**：後續採用「以大功能為單位」的分支策略，在核心功#### 二、基礎架構規劃
...
|------|------------------------|-------|
| 1    | 這是範例 |  V  |
| 2    | 完成專案權限改為 public | ![image](https://hackmd.io/_uploads/Sy8p8kmzMl.png)
 |
...
113820027 劉育維:
我真的很喜歡這門課程，並且在其中也學到非常大量的實作經驗，其中最重要的應該非Git莫屬了。之前只有處理小型專案時，git都隨邊用，有就好，甚至只有一條分支樹，完全把他當作自動存檔的工具。然而在這學期的~~痛苦~~實作中，才瞭解到git的真正用法，以及在不同情況下會產生的問題，當然還有遇到一些情況時該如何解決。
這次的專案，我自認基底架構歲是非常完美的，雖然OOP並不是現表現今最好的架構方式，然而他給的是一種觀念，不管在哪個架構之下，這些架構觀都是相當重要的一部分。握認為OOP最厲害的部分就在於他的繼承以及多型轉換想法，可以大量降低重複代碼的情形，從而提高未來DEBUG或是維護時的效率。在這次的專案之中，我並沒有完全依照OOP架構，而是在其之上採用了部份的ECS架構(雖然說實在的，就只是類似config files的概念而已，畢竟本身與OOP還是有點像是光譜的兩端)。
除此之外，我覺得還有一點是非常重要的，就是團隊合作能力，尤其是排版的部分。雖然說可以嚴格規定排版規則，但還是難免在某些情況之下會有些衝突。曾經就因為看不慣排版，強行修改，結果就是大量conflicts~~當然我沒有承認是我的問題XD~~。經歷過幾次的慘痛教訓，也就真正體會到只要能正常運作，就不要隨便修改。真的有需要時，再留到重構處理就好。
單然學到的內容遠遠不只這些，但如果要全部列出來，我認為即使萬字也說不清。總而言之，我認為這次的實作是非常難得的一次經驗，就如真的在模擬未來進入業界實操時，會遇到的各種情境，所以是個非常寶貴的練習機會。
