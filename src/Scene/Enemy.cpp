...
void Enemy::OnHeal(HpValue amount) { Character::OnHeal(amount); }
void Enemy::OnDeath() {
    Character::OnDeath();
}
...
void Enemy::AIUpdate(const std::vector<std::unique_ptr<Hero>> &heroes) {
  ...
  if (m_Target) {
    if (m_Target->IsDead()) {
      ...
      Core::Velocity intendedMovement =
          m_Target->GetWorldPosition() - GetWorldPosition();
      ...
      if (len >= m_STOP_PURSUIT_DISTANCE) {
        ...
        SetIntendedMovement({0.0f, 0.0f});
      }
    }
  }
}

void Enemy::Update() { Character::Update(); }
...
void Enemy::FindTarget(const std::vector<std::unique_ptr<Hero>> &heroes) {
  ...
  for (const auto &hero : heroes) {
    if (!hero->IsDead()) {
      Core::Distance dis =
          glm::length(hero->GetWorldPosition() - GetWorldPosition());
      if (dis < minDis) {
        minDis = dis;
        m_Target = hero.get();
      }
